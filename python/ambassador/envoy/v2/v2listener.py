...
from .v2route import DictifiedV2Route, V2RouteVariants, v2prettyroute, hostglob_matches
...
class V2Listener(dict):
    ...
    def add_vhost(self, name: str, host: 'IRHost', secure: bool) -> None:
        ...
        if self._log_debug:
            self.config.ir.logger.debug("V2Listener %s: adding %s '%s' %s)" %
    ...
    def finalize_vhosts(self) -> None:
        ...
        for host in sorted(self.config.ir.get_hosts(), key=lambda h: h.hostname or "*"):
            ...
            if not ((security_model == "SECURE") and will_reject_secure):
                self.config.ir.logger.info("V2Listener %s: take SECURE %s", self.name, host)
            ...
            if not ((security_model == "INSECURE") and (insecure_action == "Reject")):
                self.config.ir.logger.info("V2Listener %s: take INSECURE %s", self.name, host)
    ...
    def finalize_routes(self) -> None:
        ...
        # we use the V2RouteVariants to lazily cache some of the work that we're doing
        # across Hosts.
